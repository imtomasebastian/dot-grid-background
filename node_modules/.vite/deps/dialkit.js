...
import {
  require_jsx_runtime
} from "./chunk-MDPXZ5UT.js";
import {
  require_react_dom
} from "./chunk-KTSC53L7.js";
import {
  __require,
  __toESM,
  require_react
} from "./chunk-Q3ALM6WX.js";

// node_modules/dialkit/dist/index.js
...
var DialStoreClass = class {
  ...
  registerPanel(id3, name, config, shortcuts, options = {}) {
    var _a2;
    this.configurePanelRetention(id3, options);
    this.registrationCounts.set(id3, (this.registrationCounts.get(id3) ?? 0) + 1);
    const controls = this.parseConfig(config, "", shortcuts);
    const controlsByPath = this.mapControlsByPath(controls);
    const defaultValues = this.flattenValues(config, "");
    this.initTransitionModes(config, "", defaultValues);
    const persisted = this.loadPersistedPanel(id3);
    const previousValues = ((_a2 = this.panels.get(id3)) == null ? void 0 : _a2.values) ?? this.snapshots.get(id3) ?? (persisted == null ? void 0 : persisted.values) ?? {};
    const values = this.reconcileValues(defaultValues, previousValues, controlsByPath);
    const previousBaseValues = this.baseValues.get(id3) ?? (persisted == null ? void 0 : persisted.baseValues) ?? (persisted == null ? void 0 : persisted.values) ?? {};
    const baseValues = this.reconcileValues(defaultValues, previousBaseValues, controlsByPath);
    this.panels.set(id3, { id: id3, name, controls, values, shortcuts: shortcuts ?? {} });
    this.snapshots.set(id3, { ...values });
    this.baseValues.set(id3, baseValues);
    this.defaultValues.set(id3, { ...defaultValues });
    const existingPresets = this.presets.get(id3) ?? (persisted == null ? void 0 : persisted.presets);
    if (existingPresets) {
      this.presets.set(id3, this.reconcilePresets(existingPresets, defaultValues, controlsByPath));
    }
    if (!this.activePreset.has(id3) && (persisted == null ? void 0 : persisted.activePresetId) !== void 0) {
      this.activePreset.set(id3, persisted.activePresetId);
    }
    this.persistPanel(id3);
    this.notify(id3);
    this.notifyGlobal();
  }
  updatePanel(id3, name, config, shortcuts, options = {}) {
    this.configurePanelRetention(id3, options);
    const existing = this.panels.get(id3);
    if (!existing) {
      this.registerPanel(id3, name, config, shortcuts, options);
      return;
    }
    const controls = this.parseConfig(config, "", shortcuts);
    const controlsByPath = this.mapControlsByPath(controls);
    ...
    const nextPanel = { id: id3, name, controls, values: nextValues, shortcuts: shortcuts ?? existing.shortcuts };
    this.panels.set(id3, nextPanel);
    this.snapshots.set(id3, { ...nextValues });
    const previousBaseValues = this.baseValues.get(id3) ?? {};
    ...
    this.baseValues.set(id3, nextBaseValues);
    this.defaultValues.set(id3, { ...defaultValues });
    this.presets.set(id3, this.reconcilePresets(this.presets.get(id3) ?? [], defaultValues, controlsByPath));
    this.persistPanel(id3);
    this.notify(id3);
    this.notifyGlobal();
  }
  unregisterPanel(id3) {
    const nextCount = (this.registrationCounts.get(id3) ?? 1) - 1;
    if (nextCount > 0) {
      this.registrationCounts.set(id3, nextCount);
      return;
    }
    this.registrationCounts.delete(id3);
    this.panels.delete(id3);
    this.listeners.delete(id3);
    this.actionListeners.delete(id3);
    if (!this.retainedPanels.has(id3)) {
      this.snapshots.delete(id3);
      this.baseValues.delete(id3);
      this.defaultValues.delete(id3);
      this.presets.delete(id3);
      this.activePreset.delete(id3);
      this.persistConfigs.delete(id3);
    }
    this.notifyGlobal();
  }
  ...
  updateValues(panelId, updates) {
    ...
    for (const [path, value] of Object.entries(updates)) {
      ...
      if ((control == null ? void 0 : control.type) === "action") {
        continue;
      }
    ...
    if (activeId) {
      const presets2 = this.presets.get(panelId) ?? [];
      const preset = presets2.find((p) => p.id === activeId);
      if (preset) {
  ...
  getValue(panelId, path) {
    const panel = this.panels.get(panelId);
    return panel == null ? void 0 : panel.values[path];
  }
  getValues(panelId) {
  ...
  subscribe(panelId, listener) {
    ...
    return () => {
      var _a2;
      (_a2 = this.listeners.get(panelId)) == null ? void 0 : _a2.delete(listener);
    };
  }
  ...
  subscribeActions(panelId, listener) {
    ...
    return () => {
      var _a2;
      (_a2 = this.actionListeners.get(panelId)) == null ? void 0 : _a2.delete(listener);
    };
  }
  triggerAction(panelId, path) {
    var _a2;
    (_a2 = this.actionListeners.get(panelId)) == null ? void 0 : _a2.forEach((fn) => fn(path));
  }
  savePreset(panelId, name) {
    const panel = this.panels.get(panelId);
    if (!panel) throw new Error(`Panel ${panelId} not found`);
    const id3 = `preset-${Date.now()}-${Math.random().toString(36).slice(2, 9)}`;
    const preset = {
      id: id3,
      name,
      values: { ...panel.values }
    ...
    this.activePreset.set(panelId, id3);
    this.snapshots.set(panelId, { ...panel.values });
    this.persistPanel(panelId);
    this.notify(panelId);
    return id3;
  ...
  loadPreset(panelId, presetId) {
    ...
    if (!panel) return;
    const presets2 = this.presets.get(panelId) ?? [];
    const preset = presets2.find((p) => p.id === presetId);
    if (!preset) return;
  ...
  deletePreset(panelId, presetId) {
    const presets2 = this.presets.get(panelId) ?? [];
    this.presets.set(panelId, presets2.filter((p) => p.id !== presetId));
  ...
  configurePanelRetention(id3, options) {
    if (options.retainOnUnmount) {
      this.retainedPanels.add(id3);
    }
    const persistConfig = this.normalizePersistConfig(id3, options.persist);
    if (persistConfig) {
      this.persistConfigs.set(id3, persistConfig);
      this.retainedPanels.add(id3);
    }
  }
  reconcileValues(defaultValues, previousValues, controlsByPath) {
    ...
    for (const [path, defaultValue] of Object.entries(defaultValues)) {
      if (path.endsWith(".__mode")) {
        ...
        nextValues[path] = (transitionControl == null ? void 0 : transitionControl.type) === "transition" && previousValues[path] !== void 0 ? previousValues[path] : defaultValue;
        continue;
      }
  ...
  reconcilePresets(presets2, defaultValues, controlsByPath) {
    return presets2.map((preset) => ({
      ...preset,
      values: this.reconcileValues(defaultValues, preset.values, controlsByPath)
    }));
  }
  normalizePersistConfig(id3, persist) {
    if (!persist) return null;
    const options = typeof persist === "object" ? persist : {};
    return {
      key: options.key ?? `dialkit:${id3}`,
      storage: options.storage ?? "localStorage",
      presets: options.presets ?? true
    };
  }
  loadPersistedPanel(id3) {
    const config = this.persistConfigs.get(id3);
    ...
    try {
      ...
      if ((parsed == null ? void 0 : parsed.version) !== 1 || typeof parsed !== "object") return null;
      return parsed;
    } catch {
      return null;
    }
  ...
  persistPanel(id3) {
    ...
    const storage = this.getStorage(config.storage);
    if (!storage) return;
    const values = this.snapshots.get(id3) ?? ((_a2 = this.panels.get(id3)) == null ? void 0 : _a2.values);
    if (!values) return;
    const state = {
      version: 1,
      values,
      baseValues: this.baseValues.get(id3) ?? values,
      activePresetId: this.activePreset.get(id3) ?? null
    };
    if (config.presets) {
      state.presets = this.presets.get(id3) ?? [];
    }
    try {
  ...
  getStorage(kind) {
    ...
    try {
      return kind === "sessionStorage" ? ((_a2 = globalThis.window) == null ? void 0 : _a2.sessionStorage) ?? null : ((_b = globalThis.window) == null ? void 0 : _b.localStorage) ?? null;
    } catch {
      return null;
    }
  }
  ...
  notify(panelId) {
    var _a2;
    (_a2 = this.listeners.get(panelId)) == null ? void 0 : _a2.forEach((fn) => fn());
  }
  notifyGlobal() {
  ...
  parseConfig(config, prefix, shortcuts) {
    ...
    for (const [key, value] of Object.entries(config)) {
      ...
      const shortcut = shortcuts == null ? void 0 : shortcuts[path];
...
function useDialKitController(name, config, options) {
  const instanceId = (0, import_react51.useId)();
  const hasStableId = (options == null ? void 0 : options.id) !== void 0;
  const panelId = (options == null ? void 0 : options.id) ?? `${name}-${instanceId}`;
  const configRef = (0, import_react51.useRef)(config);
  const serializedConfig = JSON.stringify(config);
  configRef.current = config;
  const onActionRef = (0, import_react51.useRef)(options == null ? void 0 : options.onAction);
  onActionRef.current = options == null ? void 0 : options.onAction;
  const shortcutsRef = (0, import_react51.useRef)(options == null ? void 0 : options.shortcuts);
  shortcutsRef.current = options == null ? void 0 : options.shortcuts;
  const persistRef = (0, import_react51.useRef)(options == null ? void 0 : options.persist);
  persistRef.current = options == null ? void 0 : options.persist;
  const serializedShortcuts = JSON.stringify(options == null ? void 0 : options.shortcuts);
  const serializedPersist = JSON.stringify(options == null ? void 0 : options.persist);
  ...
  const mountedRef = (0, import_react51.useRef)(false);
  (0, import_react51.useEffect)(() => {
  ...
  (0, import_react51.useEffect)(() => {
    return DialStore.subscribeActions(panelId, (action) => {
      var _a2;
      (_a2 = onActionRef.current) == null ? void 0 : _a2.call(onActionRef, action);
    });
  }, [panelId]);
  const subscribe = (0, import_react51.useCallback)(
    (callback) => DialStore.subscribe(panelId, callback),
    [panelId]
  );
  const getSnapshot = (0, import_react51.useCallback)(
    () => DialStore.getValues(panelId),
    [panelId]
  );
  const flatValues = (0, import_react51.useSyncExternalStore)(subscribe, getSnapshot, getSnapshot);
  const values = (0, import_react51.useMemo)(
    () => resolveDialValues(configRef.current, flatValues),
    [flatValues, serializedConfig]
  );
  const setValue = (0, import_react51.useCallback)(
    (path, value) => {
      DialStore.updateValue(panelId, path, value);
    },
    [panelId]
  );
  const setValues = (0, import_react51.useCallback)(
    (nextValues) => {
      DialStore.updateValues(panelId, flattenDialValueUpdates(configRef.current, nextValues));
    },
    [panelId]
  );
  const resetValues = (0, import_react51.useCallback)(() => {
    DialStore.resetValues(panelId);
  }, [panelId]);
  const getValues = (0, import_react51.useCallback)(
...
function Folder({ title, children, defaultOpen = true, isRoot = false, inline = false, onOpenChange, toolbar, panelHeightOffset = 10 }) {
  const [isOpen, setIsOpen] = (0, import_react53.useState)(defaultOpen);
  const [isCollapsed, setIsCollapsed] = (0, import_react53.useState)(!defaultOpen);
  const contentRef = (0, import_react53.useRef)(null);
  const [contentHeight, setContentHeight] = (0, import_react53.useState)(void 0);
  const [windowHeight, setWindowHeight] = (0, import_react53.useState)(typeof window !== "undefined" ? window.innerHeight : 800);
  ...
  const folderContent = (0, import_jsx_runtime12.jsxs)(
    "div",
    {
      ...
      children: [
        (0, import_jsx_runtime12.jsxs)("div", { className: `dialkit-folder-header ${isRoot ? "dialkit-panel-header" : ""}`, onClick: handleToggle, children: [
          (0, import_jsx_runtime12.jsxs)("div", { className: "dialkit-folder-header-top", children: [
            isRoot ? isOpen && (0, import_jsx_runtime12.jsx)("div", { className: "dialkit-folder-title-row", children: (0, import_jsx_runtime12.jsx)("span", { className: "dialkit-folder-title dialkit-folder-title-root", children: title }) }) : (0, import_jsx_runtime12.jsx)("div", { className: "dialkit-folder-title-row", children: (0, import_jsx_runtime12.jsx)("span", { className: "dialkit-folder-title", children: title }) }),
            isRoot && !inline && (0, import_jsx_runtime12.jsxs)(
              "svg",
              {
                ...
                fill: "none",
                children: [
                  (0, import_jsx_runtime12.jsx)("path", { opacity: "0.5", d: ICON_PANEL.path, fill: "currentColor" }),
                  ICON_PANEL.circles.map((c, i) => (0, import_jsx_runtime12.jsx)("circle", { cx: c.cx, cy: c.cy, r: c.r, fill: "currentColor", stroke: "currentColor", strokeWidth: "1.25" }, i))
                ]
              }
            ),
            !isRoot && (0, import_jsx_runtime12.jsx)(
              motion2.svg,
              {
                ...
                children: (0, import_jsx_runtime12.jsx)("path", { d: ICON_CHEVRON })
              }
            )
          ] }),
          isRoot && toolbar && isOpen && (0, import_jsx_runtime12.jsx)("div", { className: "dialkit-panel-toolbar", onClick: (e) => e.stopPropagation(), children: toolbar })
        ] }),
        (0, import_jsx_runtime12.jsx)(AnimatePresence, { initial: false, children: isOpen && (0, import_jsx_runtime12.jsx)(
          motion2.div,
          {
            ...
            children: (0, import_jsx_runtime12.jsx)("div", { className: "dialkit-folder-inner", children })
          }
        ) })
      ]
    }
  ...
  if (isRoot) {
    if (inline) {
      return (0, import_jsx_runtime12.jsx)("div", { className: "dialkit-panel-inner dialkit-panel-inline", children: folderContent });
    }
...
var ShortcutContext = (0, import_react57.createContext)({ activePanelId: null, activePath: null });
function ShortcutListener({ children }) {
  const [activeShortcut, setActiveShortcut] = (0, import_react57.useState)({ activePanelId: null, activePath: null });
  const activeKeysRef = (0, import_react57.useRef)(/* @__PURE__ */ new Set());
  const isDraggingRef = (0, import_react57.useRef)(false);
  const lastMouseXRef = (0, import_react57.useRef)(null);
  const dragAccumulatorRef = (0, import_react57.useRef)(0);
  const resolveActiveTarget = (0, import_react57.useCallback)((interaction) => {
    ...
    const handleWheel = (e) => {
      ...
      if (activeKeysRef.current.size > 0) {
        for (const key of activeKeysRef.current) {
          ...
          const interaction = ((_a2 = control.shortcut) == null ? void 0 : _a2.interaction) ?? "scroll";
    ...
    const handleMouseMove = (e) => {
      ...
      if (isDraggingRef.current) {
        ...
        if (target && lastMouseXRef.current !== null) {
          ...
          const steps2 = Math.trunc(dragAccumulatorRef.current / DRAG_SENSITIVITY);
          if (steps2 !== 0) {
            dragAccumulatorRef.current -= steps2 * DRAG_SENSITIVITY;
            applySliderDelta(target.panelId, target.path, target.control, effectiveStep, steps2);
          }
        }
        return;
      }
      ...
      if (moveTarget) {
        ...
        if (steps2 !== 0) {
          dragAccumulatorRef.current -= steps2 * DRAG_SENSITIVITY;
          applySliderDelta(moveTarget.panelId, moveTarget.path, moveTarget.control, effectiveStep, steps2);
        }
      }
    };
  ...
  return (0, import_jsx_runtime13.jsx)(ShortcutContext.Provider, { value: activeShortcut, children });
}
...
function Slider({
  ...
  const trackRef = (0, import_react58.useRef)(null);
  const inputRef = (0, import_react58.useRef)(null);
  const labelRef = (0, import_react58.useRef)(null);
  const valueSpanRef = (0, import_react58.useRef)(null);
  const [isInteracting, setIsInteracting] = (0, import_react58.useState)(false);
  const [isDragging2, setIsDragging] = (0, import_react58.useState)(false);
  const [isHovered, setIsHovered] = (0, import_react58.useState)(false);
  const [isValueHovered, setIsValueHovered] = (0, import_react58.useState)(false);
  const [isValueEditable, setIsValueEditable] = (0, import_react58.useState)(false);
  const [showInput, setShowInput] = (0, import_react58.useState)(false);
  const [inputValue, setInputValue] = (0, import_react58.useState)("");
  const hoverTimeoutRef = (0, import_react58.useRef)(null);
  const pointerDownPos = (0, import_react58.useRef)(null);
  const isClickRef = (0, import_react58.useRef)(true);
  const animRef = (0, import_react58.useRef)(null);
  const wrapperRectRef = (0, import_react58.useRef)(null);
  const scaleRef = (0, import_react58.useRef)(1);
  ...
  const positionToValue = (0, import_react58.useCallback)(
    (clientX) => {
      ...
      const percent2 = Math.max(0, Math.min(1, sceneX / nativeWidth));
      const rawValue = min + percent2 * (max - min);
      return Math.max(min, Math.min(max, rawValue));
    },
    [min, max, value]
  );
  const percentFromValue = (0, import_react58.useCallback)(
    (v) => (v - min) / (max - min) * 100,
    [min, max]
  );
  const computeRubberStretch = (0, import_react58.useCallback)(
    (clientX, sign) => {
      const rect = wrapperRectRef.current;
      if (!rect) return 0;
  ...
  const handlePointerDown = (0, import_react58.useCallback)(
    (e) => {
      if (showInput) return;
      e.preventDefault();
  ...
  const handlePointerMove = (0, import_react58.useCallback)(
    (e) => {
      if (!isInteracting || !pointerDownPos.current) return;
      const dx = e.clientX - pointerDownPos.current.x;
      const dy = e.clientY - pointerDownPos.current.y;
      const distance2 = Math.sqrt(dx * dx + dy * dy);
      if (isClickRef.current && distance2 > CLICK_THRESHOLD) {
  ...
  const handlePointerUp = (0, import_react58.useCallback)(
    (e) => {
  ...
  const trackWidth = (_a2 = wrapperRef.current) == null ? void 0 : _a2.offsetWidth;
  ...
  const handleOpacity = !isActive ? 0 : valueDodge ? 0.1 : isDragging2 ? 0.9 : 0.5;
  ...
  return (0, import_jsx_runtime14.jsx)("div", { ref: wrapperRef, className: "dialkit-slider-wrapper", children: (0, import_jsx_runtime14.jsxs)(
    motion2.div,
    {
      ref: trackRef,
      ...
      children: [
        (0, import_jsx_runtime14.jsx)("div", { className: "dialkit-slider-hashmarks", children: hashMarks }),
        ...
        (0, import_jsx_runtime14.jsxs)("span", { ref: labelRef, className: "dialkit-slider-label", children: [
          label,
          shortcut && (0, import_jsx_runtime14.jsx)("span", { className: `dialkit-shortcut-pill${shortcutActive ? " dialkit-shortcut-pill-active" : ""}`, children: formatSliderShortcut(shortcut) })
        ] }),
        showInput ? (0, import_jsx_runtime14.jsx)(
          "input",
          {
            ref: inputRef,
            type: "text",
...
function SegmentedControl({
  ...
  const hasAnimated = (0, import_react60.useRef)(false);
  const [pillStyle, setPillStyle] = (0, import_react60.useState)(null);
  const measure = (0, import_react60.useCallback)(() => {
  ...
  return (0, import_jsx_runtime15.jsxs)("div", { className: "dialkit-segmented", ref: containerRef, children: [
    pillStyle && (0, import_jsx_runtime15.jsx)(
      "div",
      {
...
function Toggle({ label, checked, onChange, shortcut, shortcutActive }) {
  return (0, import_jsx_runtime16.jsxs)("div", { className: "dialkit-labeled-control", children: [
    (0, import_jsx_runtime16.jsxs)("span", { className: "dialkit-labeled-control-label", children: [
      label,
      shortcut && (0, import_jsx_runtime16.jsx)("span", { className: `dialkit-shortcut-pill${shortcutActive ? " dialkit-shortcut-pill-active" : ""}`, children: formatToggleShortcut(shortcut) })
    ] }),
...
function generateSpringCurve(stiffness, damping, mass, duration) {
  const points = [];
  const steps2 = 100;
  const dt = duration / steps2;
  let position = 0;
  let velocity = 0;
  const target = 1;
  for (let i = 0; i <= steps2; i++) {
    const time2 = i * dt;
    points.push([time2, position]);
...
function SpringVisualization({ spring: spring2, isSimpleMode }) {
  const width = 256;
  const height = 140;
  let stiffness;
  let damping;
  let mass;
  if (isSimpleMode) {
    const visualDuration = spring2.visualDuration ?? 0.3;
    const bounce = spring2.bounce ?? 0.2;
    mass = 1;
    ...
    stiffness = spring2.stiffness ?? 400;
    damping = spring2.damping ?? 17;
    mass = spring2.mass ?? 1;
  }
  const duration = 2;
  ...
  const pathData = points.map(([time2, value], i) => {
    const x = time2 / duration * width;
  ...
  for (let i = 1; i < 4; i++) {
    ...
    const y = height / 4 * i;
    gridLines.push(
      (0, import_jsx_runtime17.jsx)("line", { x1: x, y1: 0, x2: x, y2: height, stroke: "rgba(255, 255, 255, 0.08)", strokeWidth: "1" }, `v-${i}`),
      (0, import_jsx_runtime17.jsx)("line", { x1: 0, y1: y, x2: width, y2: y, stroke: "rgba(255, 255, 255, 0.08)", strokeWidth: "1" }, `h-${i}`)
    );
  }
  return (0, import_jsx_runtime17.jsxs)("svg", { viewBox: `0 0 ${width} ${height}`, className: "dialkit-spring-viz", children: [
    gridLines,
...
function SpringControl({ panelId, path, label, spring: spring2, onChange }) {
  const subscribe = (0, import_react61.useCallback)(
    (callback) => DialStore.subscribe(panelId, callback),
    [panelId]
  );
  const getSnapshot = (0, import_react61.useCallback)(
    () => DialStore.getSpringMode(panelId, path),
    [panelId, path]
  );
  const mode = (0, import_react61.useSyncExternalStore)(subscribe, getSnapshot, getSnapshot);
  const isSimpleMode = mode === "simple";
  const cache = (0, import_react61.useRef)({
    simple: spring2.visualDuration !== void 0 ? spring2 : { type: "spring", visualDuration: 0.3, bounce: 0.2 },
    advanced: spring2.stiffness !== void 0 ? spring2 : { type: "spring", stiffness: 200, damping: 25, mass: 1 }
  });
  if (isSimpleMode) {
    cache.current.simple = spring2;
  } else {
    cache.current.advanced = spring2;
  }
  ...
  const handleUpdate = (key, value) => {
    if (isSimpleMode) {
      const { stiffness, damping, mass, ...rest } = spring2;
      onChange({ ...rest, [key]: value });
    } else {
      const { visualDuration, bounce, ...rest } = spring2;
      onChange({ ...rest, [key]: value });
    }
  };
  return (0, import_jsx_runtime18.jsx)(Folder, { title: label, defaultOpen: true, children: (0, import_jsx_runtime18.jsxs)("div", { style: { display: "flex", flexDirection: "column", gap: 6 }, children: [
    (0, import_jsx_runtime18.jsx)(SpringVisualization, { spring: spring2, isSimpleMode }),
    (0, import_jsx_runtime18.jsxs)("div", { className: "dialkit-labeled-control", children: [
      (0, import_jsx_runtime18.jsx)("span", { className: "dialkit-labeled-control-label", children: "Type" }),
    ...
    isSimpleMode ? (0, import_jsx_runtime18.jsxs)(import_jsx_runtime18.Fragment, { children: [
      (0, import_jsx_runtime18.jsx)(
        Slider,
        {
          label: "Duration",
          value: spring2.visualDuration ?? 0.3,
...
function EasingVisualization({ easing }) {
  ...
  const end = toSvg(1, 1);
  const p1 = toSvg(ease2[0], ease2[1]);
  const p2 = toSvg(ease2[2], ease2[3]);
  ...
  return (0, import_jsx_runtime19.jsxs)(
    "svg",
    {
      ...
      children: [
        ...
        (0, import_jsx_runtime19.jsx)("path", { d: curvePath, fill: "none", stroke: "rgba(255, 255, 255, 0.6)", strokeWidth: "2", strokeLinecap: "round" })
      ]
    }
  );
}
function TransitionControl({ panelId, path, label, value, onChange }) {
  const subscribe = (0, import_react62.useCallback)(
    (callback) => DialStore.subscribe(panelId, callback),
    [panelId]
  );
  const getSnapshot = (0, import_react62.useCallback)(
    () => DialStore.getTransitionMode(panelId, path),
    [panelId, path]
  );
  const mode = (0, import_react62.useSyncExternalStore)(subscribe, getSnapshot, getSnapshot);
  const isEasing = mode === "easing";
  const isSimpleSpring = mode === "simple";
  const cache = (0, import_react62.useRef)({
  ...
  const spring2 = value.type === "spring" ? value : cache.current.simple;
  ...
  return (0, import_jsx_runtime20.jsx)(Folder, { title: label, defaultOpen: true, children: (0, import_jsx_runtime20.jsxs)("div", { style: { display: "flex", flexDirection: "column", gap: 6 }, children: [
    isEasing ? (0, import_jsx_runtime20.jsx)(EasingVisualization, { easing }) : (0, import_jsx_runtime20.jsx)(SpringVisualization, { spring: spring2, isSimpleMode: isSimpleSpring }),
    (0, import_jsx_runtime20.jsxs)("div", { className: "dialkit-labeled-control", children: [
      (0, import_jsx_runtime20.jsx)("span", { className: "dialkit-labeled-control-label", children: "Type" }),
    ...
    isEasing ? (0, import_jsx_runtime20.jsxs)(import_jsx_runtime20.Fragment, { children: [
      (0, import_jsx_runtime20.jsx)(Slider, { label: "x1", value: easing.ease[0], onChange: (v) => updateEase(0, v), min: 0, max: 1, step: 0.01 }),
      (0, import_jsx_runtime20.jsx)(Slider, { label: "y1", value: easing.ease[1], onChange: (v) => updateEase(1, v), min: -1, max: 2, step: 0.01 }),
      (0, import_jsx_runtime20.jsx)(Slider, { label: "x2", value: easing.ease[2], onChange: (v) => updateEase(2, v), min: 0, max: 1, step: 0.01 }),
      (0, import_jsx_runtime20.jsx)(Slider, { label: "y2", value: easing.ease[3], onChange: (v) => updateEase(3, v), min: -1, max: 2, step: 0.01 }),
      (0, import_jsx_runtime20.jsx)(Slider, { label: "Duration", value: easing.duration, onChange: (v) => onChange({ ...easing, duration: v }), min: 0.1, max: 2, step: 0.05, unit: "s" }),
      (0, import_jsx_runtime20.jsx)(EaseTextInput, { ease: easing.ease, onChange: (newEase) => onChange({ ...easing, ease: newEase }) })
    ] }) : isSimpleSpring ? (0, import_jsx_runtime20.jsxs)(import_jsx_runtime20.Fragment, { children: [
      (0, import_jsx_runtime20.jsx)(Slider, { label: "Duration", value: spring2.visualDuration ?? 0.3, onChange: (v) => handleSpringUpdate("visualDuration", v), min: 0.1, max: 1, step: 0.05, unit: "s" }),
      (0, import_jsx_runtime20.jsx)(Slider, { label: "Bounce", value: spring2.bounce ?? 0.2, onChange: (v) => handleSpringUpdate("bounce", v), min: 0, max: 1, step: 0.05 })
    ] }) : (0, import_jsx_runtime20.jsxs)(import_jsx_runtime20.Fragment, { children: [
      (0, import_jsx_runtime20.jsx)(Slider, { label: "Stiffness", value: spring2.stiffness ?? 400, onChange: (v) => handleSpringUpdate("stiffness", v), min: 1, max: 1e3, step: 10 }),
      (0, import_jsx_runtime20.jsx)(Slider, { label: "Damping", value: spring2.damping ?? 17, onChange: (v) => handleSpringUpdate("damping", v), min: 1, max: 100, step: 1 }),
      (0, import_jsx_runtime20.jsx)(Slider, { label: "Mass", value: spring2.mass ?? 1, onChange: (v) => handleSpringUpdate("mass", v), min: 0.1, max: 10, step: 0.1 })
    ] })
  ] }) });
}
function formatEase(ease2) {
  return ease2.map((v) => parseFloat(v.toFixed(2))).join(", ");
}
...
function EaseTextInput({ ease: ease2, onChange }) {
  const [editing, setEditing] = (0, import_react62.useState)(false);
  const [draft, setDraft] = (0, import_react62.useState)("");
  const handleFocus = () => {
    setDraft(formatEase(ease2));
    setEditing(true);
  };
  const handleBlur = () => {
  ...
  return (0, import_jsx_runtime20.jsxs)("div", { className: "dialkit-labeled-control", children: [
    (0, import_jsx_runtime20.jsx)("span", { className: "dialkit-labeled-control-label", children: "Ease" }),
    (0, import_jsx_runtime20.jsx)(
      "input",
      {
        type: "text",
        className: "dialkit-text-input",
        value: editing ? draft : formatEase(ease2),
...
function TextControl({ label, value, onChange, placeholder }) {
  return (0, import_jsx_runtime21.jsxs)("div", { className: "dialkit-text-control", children: [
    (0, import_jsx_runtime21.jsx)("label", { className: "dialkit-text-label", children: label }),
...
function getDialKitPortalRoot(trigger) {
  return (trigger == null ? void 0 : trigger.closest(".dialkit-root")) ?? null;
}
...
function SelectControl({ label, value, options, onChange }) {
  ...
  const triggerRef = (0, import_react63.useRef)(null);
  const dropdownRef = (0, import_react63.useRef)(null);
  const [portalTarget, setPortalTarget] = (0, import_react63.useState)(null);
  const [pos, setPos] = (0, import_react63.useState)(null);
  const normalized = normalizeOptions(options);
  const selectedOption = normalized.find((o) => o.value === value);
  const updatePos = (0, import_react63.useCallback)(() => {
  ...
  return (0, import_jsx_runtime22.jsxs)("div", { className: "dialkit-select-row", children: [
    (0, import_jsx_runtime22.jsxs)(
      "button",
      {
        ...
        "data-open": String(isOpen),
        children: [
          (0, import_jsx_runtime22.jsx)("span", { className: "dialkit-select-label", children: label }),
          (0, import_jsx_runtime22.jsxs)("div", { className: "dialkit-select-right", children: [
            (0, import_jsx_runtime22.jsx)("span", { className: "dialkit-select-value", children: (selectedOption == null ? void 0 : selectedOption.label) ?? value }),
            (0, import_jsx_runtime22.jsx)(
              motion2.svg,
              {
                ...
                children: (0, import_jsx_runtime22.jsx)("path", { d: ICON_CHEVRON })
              }
            )
          ] })
        ]
    ...
    portalTarget && (0, import_react_dom2.createPortal)(
      (0, import_jsx_runtime22.jsx)(AnimatePresence, { children: isOpen && pos && (0, import_jsx_runtime22.jsx)(
        motion2.div,
        {
          ref: dropdownRef,
          ...
          children: normalized.map((option) => (0, import_jsx_runtime22.jsx)(
            "button",
            {
...
function ColorControl({ label, value, onChange }) {
  ...
  const [editValue, setEditValue] = (0, import_react65.useState)(value);
  const colorInputRef = (0, import_react65.useRef)(null);
  ...
  return (0, import_jsx_runtime23.jsxs)("div", { className: "dialkit-color-control", children: [
    (0, import_jsx_runtime23.jsx)("span", { className: "dialkit-color-label", children: label }),
    (0, import_jsx_runtime23.jsxs)("div", { className: "dialkit-color-inputs", children: [
      isEditing ? (0, import_jsx_runtime23.jsx)(
        "input",
        {
          type: "text",
      ...
      (0, import_jsx_runtime23.jsx)(
        "button",
        {
          ...
          onClick: () => {
            var _a2;
            return (_a2 = colorInputRef.current) == null ? void 0 : _a2.click();
          },
          title: "Pick color"
        }
      ),
...
function expandShorthandHex(hex2) {
  if (hex2.length !== 4) return hex2;
  return `#${hex2[1]}${hex2[1]}${hex2[2]}${hex2[2]}${hex2[3]}${hex2[3]}`;
}
function PresetManager({ panelId, presets: presets2, activePresetId, onAdd }) {
  const [isOpen, setIsOpen] = (0, import_react66.useState)(false);
  const triggerRef = (0, import_react66.useRef)(null);
  const dropdownRef = (0, import_react66.useRef)(null);
  const [pos, setPos] = (0, import_react66.useState)({ top: 0, left: 0, width: 0 });
  const hasPresets = presets2.length > 0;
  const activePreset = presets2.find((p) => p.id === activePresetId);
  const open = (0, import_react66.useCallback)(() => {
    var _a2;
    if (!hasPresets) return;
    const rect = (_a2 = triggerRef.current) == null ? void 0 : _a2.getBoundingClientRect();
    if (rect) {
      setPos({ top: rect.bottom + 4, left: rect.left, width: rect.width });
    }
    setIsOpen(true);
  }, [hasPresets]);
  const close = (0, import_react66.useCallback)(() => setIsOpen(false), []);
  ...
  (0, import_react66.useEffect)(() => {
    ...
    const handler = (e) => {
      var _a2, _b;
      const target = e.target;
      if (((_a2 = triggerRef.current) == null ? void 0 : _a2.contains(target)) || ((_b = dropdownRef.current) == null ? void 0 : _b.contains(target))) return;
      close();
    };
  ...
  return (0, import_jsx_runtime24.jsxs)("div", { className: "dialkit-preset-manager", children: [
    (0, import_jsx_runtime24.jsxs)(
      "button",
      {
        ...
        children: [
          (0, import_jsx_runtime24.jsx)("span", { className: "dialkit-preset-label", children: activePreset ? activePreset.name : "Version 1" }),
          (0, import_jsx_runtime24.jsx)(
            motion2.svg,
            {
              ...
              transition: { type: "spring", visualDuration: 0.2, bounce: 0.15 },
              children: (0, import_jsx_runtime24.jsx)("path", { d: ICON_CHEVRON })
            }
          )
        ]
      }
    ...
    (0, import_react_dom3.createPortal)(
      (0, import_jsx_runtime24.jsx)(AnimatePresence, { children: isOpen && (0, import_jsx_runtime24.jsxs)(
        motion2.div,
        {
          ref: dropdownRef,
          ...
          children: [
            (0, import_jsx_runtime24.jsx)(
              "div",
              {
                ...
                children: (0, import_jsx_runtime24.jsx)("span", { className: "dialkit-preset-name", children: "Version 1" })
              }
            ),
            presets2.map((preset) => (0, import_jsx_runtime24.jsxs)(
              "div",
              {
                className: "dialkit-preset-item",
                ...
                children: [
                  (0, import_jsx_runtime24.jsx)("span", { className: "dialkit-preset-name", children: preset.name }),
                  (0, import_jsx_runtime24.jsx)(
                    "button",
                    {
                      ...
                      title: "Delete preset",
                      children: (0, import_jsx_runtime24.jsx)("svg", { viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round", strokeLinejoin: "round", children: ICON_TRASH.map((d, i) => (0, import_jsx_runtime24.jsx)("path", { d }, i)) })
                    }
                  )
                ]
              },
...
function Panel({ panel, defaultOpen = true, inline = false, onOpenChange, variant = "root" }) {
  ...
  const [isPanelOpen, setIsPanelOpen] = (0, import_react55.useState)(defaultOpen);
  const shortcutCtx = (0, import_react55.useContext)(ShortcutContext);
  const hasShortcuts = Object.keys(panel.shortcuts).length > 0;
  const subscribe = (0, import_react55.useCallback)(
    (callback) => DialStore.subscribe(panel.id, callback),
    [panel.id]
  );
  const getSnapshot = (0, import_react55.useCallback)(
    () => DialStore.getValues(panel.id),
    [panel.id]
  );
  const values = (0, import_react55.useSyncExternalStore)(subscribe, getSnapshot, getSnapshot);
  const presets2 = DialStore.getPresets(panel.id);
  const activePresetId = DialStore.getActivePresetId(panel.id);
  const handleAddPreset = () => {
    const nextNum = presets2.length + 2;
...
Apply these values as the new defaults in the useDialKit call.`;
  ...
  const handleOpenChange = (0, import_react55.useCallback)((open) => {
    setIsPanelOpen(open);
  ...
  const renderControl = (control) => {
    ...
    switch (control.type) {
      ...
      case "folder":
        return (0, import_jsx_runtime25.jsx)(Folder, { title: control.label, defaultOpen: control.defaultOpen ?? true, children: (_a2 = control.children) == null ? void 0 : _a2.map(renderControl) }, control.path);
      case "text":
        return (0, import_jsx_runtime25.jsx)(
          TextControl,
          {
  ...
  const toolbar = (0, import_jsx_runtime25.jsxs)(import_jsx_runtime25.Fragment, { children: [
    (0, import_jsx_runtime25.jsx)(
      motion2.button,
      {
        ...
        whileTap: { scale: 0.9 },
        transition: { type: "spring", visualDuration: 0.15, bounce: 0.3 },
        children: (0, import_jsx_runtime25.jsx)("svg", { viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2.5", strokeLinecap: "round", strokeLinejoin: "round", children: ICON_ADD_PRESET.map((d, i) => (0, import_jsx_runtime25.jsx)("path", { d }, i)) })
      }
    ),
    (0, import_jsx_runtime25.jsx)(
      PresetManager,
    ...
    (0, import_jsx_runtime25.jsx)(
      motion2.button,
      {
        ...
        whileTap: { scale: 0.9 },
        transition: { type: "spring", visualDuration: 0.15, bounce: 0.3 },
        children: (0, import_jsx_runtime25.jsx)("span", { style: { position: "relative", width: 16, height: 16 }, children: (0, import_jsx_runtime25.jsx)(AnimatePresence, { initial: false, mode: "wait", children: copied ? (0, import_jsx_runtime25.jsx)(
          motion2.svg,
          {
            viewBox: "0 0 24 24",
            fill: "none",
            ...
            children: (0, import_jsx_runtime25.jsx)("path", { d: ICON_CHECK })
          },
          "check"
          ...
          motion2.svg,
          {
            ...
            transition: { duration: 0.08 },
            children: [
              (0, import_jsx_runtime25.jsx)("path", { d: ICON_CLIPBOARD.board, stroke: "currentColor", strokeWidth: "2", strokeLinejoin: "round" }),
              (0, import_jsx_runtime25.jsx)("path", { d: ICON_CLIPBOARD.sparkle, fill: "currentColor" }),
              (0, import_jsx_runtime25.jsx)("path", { d: ICON_CLIPBOARD.body, stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round", strokeLinejoin: "round" })
            ]
          },
          "clipboard"
        ) }) })
  ...
  if (variant === "section") {
    return (0, import_jsx_runtime25.jsxs)(Folder, { title: panel.name, defaultOpen, onOpenChange: handleOpenChange, children: [
      (0, import_jsx_runtime25.jsx)("div", { className: "dialkit-panel-section-toolbar", onClick: (e) => e.stopPropagation(), children: toolbar }),
      renderControls()
    ] });
  }
  return (0, import_jsx_runtime25.jsx)("div", { className: "dialkit-panel-wrapper", children: (0, import_jsx_runtime25.jsx)(Folder, { title: panel.name, defaultOpen, isRoot: true, inline, onOpenChange: handleOpenChange, toolbar, children: renderControls() }) });
}
...
var _a;
var isDevDefault = typeof process !== "undefined" && "development" ? true : typeof import.meta !== "undefined" && ((_a = import.meta.env) == null ? void 0 : _a.MODE) ? import.meta.env.MODE !== "production" : true;
function DialRoot({ position = "top-right", defaultOpen = true, mode = "popover", theme = "system", productionEnabled = isDevDefault, onOpenChange }) {
  ...
  const [panels, setPanels] = (0, import_react52.useState)([]);
  const [mounted, setMounted] = (0, import_react52.useState)(false);
  const inline = mode === "inline";
  const panelRef = (0, import_react52.useRef)(null);
  const [dragOffset, setDragOffset] = (0, import_react52.useState)(null);
  const [activePosition, setActivePosition] = (0, import_react52.useState)(position);
  const lastDragOffset = (0, import_react52.useRef)(null);
  const draggingRef = (0, import_react52.useRef)(false);
  const dragStartRef = (0, import_react52.useRef)(null);
  const didDragRef = (0, import_react52.useRef)(false);
  const dragTargetRef = (0, import_react52.useRef)(null);
  const panelOpenStatesRef = (0, import_react52.useRef)(/* @__PURE__ */ new Map());
  const rootOpenRef = (0, import_react52.useRef)(null);
  ...
  (0, import_react52.useEffect)(() => {
    ...
    const observer2 = new MutationObserver(() => {
      var _a2;
      const inners = (_a2 = panelRef.current) == null ? void 0 : _a2.querySelectorAll(".dialkit-panel-inner");
    ...
    observer2.observe(panelRef.current, { subtree: true, attributes: true, attributeFilter: ["data-collapsed"] });
    return () => observer2.disconnect();
  }, [inline, dragOffset, position]);
  const handlePointerDown = (0, import_react52.useCallback)((e) => {
  ...
  const handlePointerMove = (0, import_react52.useCallback)((e) => {
  ...
  const handlePointerUp = (0, import_react52.useCallback)((e) => {
    if (!draggingRef.current) return;
    draggingRef.current = false;
    dragStartRef.current = null;
    const dragTarget = dragTargetRef.current;
    if (dragTarget == null ? void 0 : dragTarget.hasPointerCapture(e.pointerId)) {
  ...
  const handlePanelOpenChange = (0, import_react52.useCallback)((panelId, open) => {
    ...
    onOpenChange == null ? void 0 : onOpenChange(nextRootOpen);
  }, [defaultOpen, inline, onOpenChange, panels]);
  const handleRootOpenChange = (0, import_react52.useCallback)((open) => {
  ...
  const content = (0, import_jsx_runtime26.jsx)(ShortcutListener, { children: (0, import_jsx_runtime26.jsx)("div", { className: "dialkit-root", "data-mode": mode, "data-theme": theme, children: (0, import_jsx_runtime26.jsx)(
    "div",
    {
      ref: panelRef,
      ...
      children: hasMultiplePanels ? (0, import_jsx_runtime26.jsx)("div", { className: "dialkit-panel-wrapper", children: (0, import_jsx_runtime26.jsx)(
        Folder,
        {
          title: "DialKit",
          ...
          panelHeightOffset: 2,
          children: panels.map((panel) => (0, import_jsx_runtime26.jsx)(
            Panel,
            {
              panel,
              defaultOpen: true,
      ...
      ) }) : panels.map((panel) => (0, import_jsx_runtime26.jsx)(
        Panel,
        {
          panel,
          defaultOpen: inline || defaultOpen,
...
function ButtonGroup({ buttons }) {
  return (0, import_jsx_runtime27.jsx)("div", { className: "dialkit-button-group", children: buttons.map((button, index) => (0, import_jsx_runtime27.jsx)(
    "button",
    {
      className: "dialkit-button",
...
function ShortcutsMenu({ panelId }) {
  ...
  const dropdownRef = (0, import_react68.useRef)(null);
  const [pos, setPos] = (0, import_react68.useState)({ top: 0, right: 0 });
  const open = (0, import_react68.useCallback)(() => {
    var _a2;
    const rect = (_a2 = triggerRef.current) == null ? void 0 : _a2.getBoundingClientRect();
    if (rect) {
  ...
  const close = (0, import_react68.useCallback)(() => setIsOpen(false), []);
  ...
  return (0, import_jsx_runtime28.jsxs)(import_jsx_runtime28.Fragment, { children: [
    (0, import_jsx_runtime28.jsx)(
      motion2.button,
      {
        ...
        whileTap: { scale: 0.9 },
        transition: { type: "spring", visualDuration: 0.15, bounce: 0.3 },
        children: (0, import_jsx_runtime28.jsxs)("svg", { viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round", strokeLinejoin: "round", children: [
          (0, import_jsx_runtime28.jsx)("rect", { x: "2", y: "6", width: "20", height: "12", rx: "2" }),
          (0, import_jsx_runtime28.jsx)("path", { d: "M6 10H6.01" }),
          (0, import_jsx_runtime28.jsx)("path", { d: "M10 10H10.01" }),
          (0, import_jsx_runtime28.jsx)("path", { d: "M14 10H14.01" }),
          (0, import_jsx_runtime28.jsx)("path", { d: "M18 10H18.01" }),
          (0, import_jsx_runtime28.jsx)("path", { d: "M8 14H16" })
        ] })
      }
    ),
    (0, import_react_dom4.createPortal)(
      (0, import_jsx_runtime28.jsx)(AnimatePresence, { children: isOpen && (0, import_jsx_runtime28.jsxs)(
        motion2.div,
        {
          ref: dropdownRef,
          ...
          transition: { type: "spring", visualDuration: 0.15, bounce: 0 },
          children: [
            (0, import_jsx_runtime28.jsx)("div", { className: "dialkit-shortcuts-title", children: "Keyboard Shortcuts" }),
            (0, import_jsx_runtime28.jsx)("div", { className: "dialkit-shortcuts-list", children: rows.map((row) => (0, import_jsx_runtime28.jsxs)("div", { className: "dialkit-shortcuts-row", children: [
              (0, import_jsx_runtime28.jsx)("span", { className: "dialkit-shortcuts-row-key", children: formatShortcutKey(row.shortcut) }),
              (0, import_jsx_runtime28.jsx)("span", { className: "dialkit-shortcuts-row-label", children: row.label }),
              (0, import_jsx_runtime28.jsx)("span", { className: "dialkit-shortcuts-row-mode", children: formatInteraction(row.shortcut) })
            ] }, row.path)) }),
            (0, import_jsx_runtime28.jsx)("div", { className: "dialkit-shortcuts-hint", children: "See pill badges on controls for keys" })
          ]
        }
      ) }),
      document.body
